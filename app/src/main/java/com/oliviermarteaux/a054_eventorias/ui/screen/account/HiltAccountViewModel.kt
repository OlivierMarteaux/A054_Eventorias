...
class HiltAccountViewModel @Inject constructor(
    ...
    val containerUserRepository: UserRepository by mutableStateOf ((applicationContext as EventoriasApplication).eventoriasContainer.userRepository)
...
//                .filterNotNull()
                .collect { currentUser ->
                    if (currentUser != null) {
                        userUiState = UiState.Success(currentUser)
                        this@HiltAccountViewModel.user = currentUser
                        log.d("AccountViewModel: user updated to ${currentUser.email}")
